...
@Tag(
        ...
        description = "Endpoints for user registration, login, logout, and account verification"
)
public class SecurityController {
    ...
    @Operation(
            ...
            description = "Registers a new user with the provided details. Fails if the username or email is already taken."
    )
    ...
    public ResponseEntity<@NonNull String> registerUser(
        ...
        securityServiceImpl
                .createNewUser(registerRequestDTO);
    ...
    @Operation(
            summary = "Logout user",
            description = "Logs out the currently authenticated user by invalidating their session cookie."
    )
    @PostMapping("logout")
    ...
    @Operation(
            summary = "Login user",
            description = "Authenticates a user with email and password, and sets an HTTP-only session cookie upon success."
    )
    @PostMapping("login")
    @RateLimiter(
            requestAllowed = 5,
    ...
    public ResponseEntity<@NonNull String> resetPassword(
        ...
        securityServiceImpl
                .modifyUserPassword(passwordResetRequestDTO);

        List<ResponseCookie> clearedCookie = securityServiceImpl
                .terminateSession(passwordResetRequestDTO.getEmail());
    ...
    public ResponseEntity<@NonNull String> verifyAccount(
        ...
        securityServiceImpl
                .verifyUserEmail(accountVerificationRequestDTO, userDetails.getUsername());
