package com.project.hearmeout_backend.administration_service.controller;
...
import com.project.hearmeout_backend.administration_service.dto.response.AdminTagResponseDTO;
import com.project.hearmeout_backend.administration_service.service.implementation.AdminTagServiceImpl;
...
@Tag(
    ...
    description =
        "Endpoints for fetching, creating, updating, and deleting tags used for categorizing posts by Admins")
public class AdminTagController {
  private final AdminTagServiceImpl adminTagServiceImpl;
  ...
  @GetMapping("admin")
  public ResponseEntity<PagedResponse<AdminTagResponseDTO>> adminTagList(
      @RequestParam(defaultValue = "5") int limit, @RequestParam(defaultValue = "0") int offset) {
    return ResponseEntity.status(HttpStatus.OK)
        .body(adminTagServiceImpl.getAllTagsAdmin(limit, offset));
  }
  ...
  public ResponseEntity<String> createTag(@Valid @RequestBody TagCreationRequestDTO tag) {
    adminTagServiceImpl.createNewTag(tag);
  ...
  public ResponseEntity<String> updateTag(
    ...
    adminTagServiceImpl.updateTag(tagId, tagModificationRequestDTO);
  ...
  public ResponseEntity<String> deleteTag(
      @Parameter(description = "The ID of the tag to delete", required = true) @PathVariable
          UUID tagId) {
    adminTagServiceImpl.deleteTag(tagId);
