...
import com.project.hearmeout_backend.administration_service.mapper.AdminTagMapper;
...
public class AdminTagServiceImpl {
  ...
  public PagedResponse<AdminTagResponseDTO> getAllTagsAdmin(int limit, int offset) {
    int page = offset / limit;
    Pageable pageable = PageRequest.of(Math.max(page, 0), limit);

    Page<AdminTagResponseDTO> tagPage = tagRepo.findAllTagsDTOAdmin(pageable);
    return PagedResponse.<AdminTagResponseDTO>builder()
  ...
  public void createNewTag(TagCreationRequestDTO tag) {
    tagRepo.save(AdminTagMapper.toTagEntity(tag));
  }
